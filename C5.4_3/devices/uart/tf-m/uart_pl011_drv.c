                    ...
                    uint32_t clk, uint32_t baudrate)
{
    ...
    // porting note: -cpu arm926 doesn't support DMB instruction
    // __DMB();
                    ...
                    uint32_t uart_clk)
{
    ...
    /* Enabling the FIFOs */
    /*
    _uart_pl011_enable_fifo() writes to the UARTLCR_H.
    And it must be the last one according to PL011 TRM r1p5:
